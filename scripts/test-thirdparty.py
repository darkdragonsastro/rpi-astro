...
def expected_qhy_version(bookworm, architecture, documentation=Path("/usr/share/doc/indi-3rdparty-libs")):
    if bookworm and architecture == "amd64":
        metadata = json.loads((documentation / "qhybookworm.json").read_text())
...
def check():
    ...
    expected_qhy = expected_qhy_version(bookworm, architecture)
    ...
    actual_qhy = tuple(part.value for part in version)
    assert result == 0 and actual_qhy[:len(expected_qhy)] == expected_qhy, f"Unexpected QHY SDK: {actual_qhy}"
    ...
    required.extend(("indi_qhy_ccd", "indi_qhy_focuser", "qhy_ccd_test"))
