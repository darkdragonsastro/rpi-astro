...
QHY is the explicit target exception: both aggregate packages configure `WITH_QHY=OFF` only for Bookworm amd64, excluding the driver, SDK, headers, firmware and udev rules. The current x64 SDK requires newer runtimes than Bookworm; the attempted 26.2.1 fallback lacks the current driver's `CONTROL_CAA_ROTATOR` API, so the fallback component and patch were removed. Other targets retain QHY 26.7.21. Tests verify both packages' flags across all four targets, reject QHY payloads on Bookworm amd64, and query `GetQHYCCDSDKVersion` elsewhere. The full primary source archive and upstream notices remain preserved on all targets. Review this exception deliberately during upstream updates.
