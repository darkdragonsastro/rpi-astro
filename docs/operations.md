...
CI starts with Debian's `indi-bin` and `kstars`, upgrades to the built packages, checks dependencies and shared-library loading, runs `kstars --version` with an offscreen Qt backend, and queries an INDI simulator. Repository tests exercise signatures and APT downloads independently.
