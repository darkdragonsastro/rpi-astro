...
QHY is the explicit target exception starting with revision 6: Bookworm amd64 uses the official vendor SDK **26.7.28.15**, while other targets retain bundled **26.7.21**. Only the library aggregate configures `WITH_QHY=OFF` on Bookworm amd64 to skip the incompatible bundled SDK; `debian/install-qhy.py` stages the vendor library, matching headers (including `config.h`), firmware and rules instead. The driver aggregate enables QHY on every target and uses the installed SDK through upstream's normal discovery, with no QHY driver patch. Existing firmware-loader and udev safety adaptations still apply.

The `qhybookworm` component in `sources.json` pins the official HTTPS archive, full SDK version and SHA-256 checksums of both archive and library. The complete, byte-for-byte vendor tarball is included as a supplementary orig archive in the library source package on every target, including the canonical ARM64 source builds; rebuilding downloaded source does not need a vendor download. Its metadata is installed as `/usr/share/doc/indi-3rdparty-libs/qhybookworm.json`, with upstream QHY notices preserved under `upstream-notices/debian/libqhy/copyright`. No vendor installer, static SDK archive or demo executable is installed. Tests check target selection, checksum rejection, source round trips, required driver/firmware payloads and the loaded SDK version (all four version fields for the vendor build). The vendor version is deliberately allowed to differ from INDI's bundled version: it remains pinned until deliberately updated; future driver incompatibilities should fail CI and be addressed then, not trigger an automatic SDK substitution. All four native builds and runtime checks are required before publication.
